...
class MLP_NeuralNetwork(object):
	...
	def test(self, patterns, y_max):
		...
		errors = []
		for p in patterns:
			nn_result = self.feedForward(p[0])[0]
			error = abs((p[1][0] * float(y_max)) - (nn_result * float(y_max)))
			errors.append(error)
			print("Actual: {}".format(p[1][0] * y_max), '->', "Network Prediction: {}".format(nn_result * y_max))
		return errors
